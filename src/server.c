...
void cursor_button(struct wl_listener *listener, void *data)
{
    ...
    if (event->state == WL_POINTER_BUTTON_STATE_RELEASED) {
        ...
        client_from_coords(server, server->cursor->x, server->cursor->y,
                           &surface, &toplevel, &layer_surface, &x, &y);
        ...
        if (server->grab_x > (tx + width / 2) &&
            server->grab_y > (ty + height / 2)) {
            server->resize_corner = BOTTOM_RIGHT;
            wlr_cursor_set_xcursor(server->cursor, server->cursor_mgr,
                                   "se-resize");
        } else if (server->grab_x < (tx + width / 2) &&
                   server->grab_y > (ty + height / 2)) {
